<template>
  <div class="flex relative w-full h-[20rem] flex-col items-center justify-center p-4 mb-12 bg-[#272727] text-[white] text-[1.5rem] rounded-[0.9rem] shadow-md">
    ...
    <p class="absolute bottom-4 p-2 text-center text-[1.4rem] border-2 rounded-[0.9rem]">{{ previewDevice.name == "" ? "Nome da máquina" : previewDevice.name }}</p>
  </div>
</template>

<script setup lang="ts">
defineProps<{
  previewDevice: Record<string, any>;
}>();
</script>
